...
constexpr int MAX_TRIANGLES = 50;
...
void exportRender(GLFWwindow* window, ObjectBuffer& objectBuffer, GLuint& VAO, GLuint& UBO, GLuint& UBOIndex, GLuint& shaderProgram, int numSamples, int maxBounces) {
	...
	std::string filename = "renders/render_" + std::to_string(numSamples) + "_" + std::to_string(maxBounces) + ".png";
	int width, height;
	...
	// Reset the number of samples and bounces
	objectBuffer.numSamples = tmpNumSamples;
	objectBuffer.maxBounces = tmpMaxBounces;
}
...
int main() {
	...
	if (loadMesh(objectBuffer, "meshes/uv_sphere.obj", meshes[0])) {
